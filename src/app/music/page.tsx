...
import { useMusicArtists } from '@/lib/hooks'
import { Search, Music2, Instagram } from 'lucide-react'
import { CardSkeleton } from '@/components/Skeleton'

export default function MusicPage() {
  const [search, setSearch] = useState('')
  const { data, isLoading } = useMusicArtists(search||undefined)
  const artists = (data as any)?.data ?? []
  const total   = (data as any)?.total ?? 0

  return (
    <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto pb-6">
      <div>
        <p className="text-[11px] text-purple-400 uppercase tracking-widest font-semibold">Music Intelligence</p>
      ...
      {isLoading?<CardSkeleton n={6}/>:artists.length===0?(
            ...
            <div key={a.id} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-3 py-3">
              ...
              {a.instagram_followers!=null&&(
                <div className="text-[11px] text-purple-400 flex-shrink-0">
                  {a.instagram_followers>=1e6?`${(a.instagram_followers/1e6).toFixed(1)}M`:`${Math.round(a.instagram_followers/1e3)}K`}
                </div>
              )}
            </div>
          ))}
