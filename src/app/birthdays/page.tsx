'use client'
import { useBirthdays } from '@/lib/hooks'
...
export default function BirthdaysPage() {
  ...
  const week  = all.filter(b=>b.days_until>0&&b.days_until<=7)
  const later = all.filter(b=>b.days_until>7)

  const Card = ({b,hi}:{b:any,hi?:boolean}) => (
    <div className={`flex items-center gap-3 rounded-xl px-3 py-3 border ${hi?'bg-yellow-500/10 border-yellow-500/20':'bg-white/5 border-white/10'}`}>
      <div className={`h-10 w-10 rounded-full flex items-center justify-center text-lg flex-shrink-0 ${hi?'bg-yellow-500/20':'bg-white/5'}`}>🎂</div>
      <div className="flex-1 min-w-0">
        <div className="text-sm font-semibold text-white truncate">{b.name}</div>
        <div className="flex items-center gap-1.5 text-[10px] text-white/40">
          <TypeIcon type={b.client_type}/>
      ...
      <div className={`text-sm font-bold px-2 py-1 rounded-lg flex-shrink-0 ${b.days_until===0?'bg-yellow-500/20 text-yellow-400':b.days_until<=3?'bg-orange-500/20 text-orange-400':'bg-white/10 text-white/40'}`}>
        {b.days_until===0?'Today!':b.days_until<=7?`${b.days_until}d`:`${b.days_until}d`}
      </div>
    </div>
  )
  ...
  return (
    <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto pb-6">
      <div>
        <p className="text-[11px] text-pink-400 uppercase tracking-widest font-semibold">Client Intelligence</p>
        <h1 className="text-2xl font-bold text-white">Birthdays</h1>
        <p className="text-white/40 text-sm">Next 30 days · {all.length} upcoming</p>
      </div>

      {isLoading?<CardSkeleton n={6}/>:all.length===0?(
        ...
        <div className="space-y-5">
          {today.length>0&&<section><p className="text-xs font-semibold text-yellow-400 uppercase tracking-wider mb-2">🎂 Today</p><div className="space-y-2">{today.map(b=><Card key={b.id} b={b} hi/>)}</div></section>}
