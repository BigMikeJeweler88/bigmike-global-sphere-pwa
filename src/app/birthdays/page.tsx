...
export default function BirthdaysPage() {
  ...
  // /api/upcoming-birthdays returns array or { data: [] }
  const birthdays: any[] = Array.isArray(raw) ? raw : (raw?.data || raw?.birthdays || [])
  ...
  const icon = (b: any) => {
    const ct = (b.client_type || b.league || '').toLowerCase()
    if (ct === 'athlete' || ['nfl','nba','mlb','nhl','mls'].some(l => ct.includes(l))) return <Trophy className="h-3 w-3 text-orange-400" />
    if (ct === 'music') return <Music2 className="h-3 w-3 text-purple-400" />
    if (ct === 'civilian') return <Users className="h-3 w-3 text-yellow-400" />
    ...
    <div className={`flex items-center gap-3 rounded-xl px-3 py-3 border ${highlight ? 'bg-yellow-500/10 border-yellow-500/20' : 'bg-white/5 border-white/10'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-[10px] text-white/40">
          ...
          {b.age && <><span>·</span><span>Turns {b.age}</span></>}
          {b.birthday && <><span>·</span><span>{b.birthday}</span></>}
        </div>
      </div>
  ...
  return (
    <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto pb-24">
      ...
      {isLoading ? (
        ...
        <>
          {today.length > 0 && <div><div className="text-xs font-semibold text-yellow-400 uppercase tracking-wider mb-2">🎂 Today</div><div className="space-y-2">{today.map(b => <Card key={b.client_id || b.id} b={b} highlight />)}</div></div>}
          {week.length > 0 && <div><div className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-2">This Week</div><div className="space-y-2">{week.map(b => <Card key={b.client_id || b.id} b={b} />)}</div></div>}
          {later.length > 0 && <div><div className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2">Later This Month</div><div className="space-y-2">{later.map(b => <Card key={b.client_id || b.id} b={b} />)}</div></div>}
        </>
      )}
    </div>
  )
