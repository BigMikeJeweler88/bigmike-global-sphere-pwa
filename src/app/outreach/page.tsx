  ...
  return (
    <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto pb-24">
      <div>
        ...
        <p className="text-white/40 text-sm">{items.length} pending approval</p>
      </div>

      <div className="grid grid-cols-3 gap-2">
        {[
          { label: 'Generated', value: s.total_generated ?? 0, color: 'text-blue-400' },
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="text-[10px] text-white/40 mt-0.5">
                    {item.persona?.replace(/_/g, ' ')} · {new Date(item.created_at).toLocaleDateString()}
                  </div>
                </div>
                <Send className="h-4 w-4 text-blue-400 flex-shrink-0"/>
              </div>
              <div className="bg-black/20 rounded-lg p-3 mb-3">
                <p className="text-xs text-white/70 leading-relaxed line-clamp-4">{item.message_text}</p>
              </div>
